...
import os from 'os';
import { BACKEND_KIT_DIR, MANIFEST_FILENAME, BekManifest } from '../lib/manifest.js';
...
describe('Remove Command', () => {
    ...
    function createMockProject(options: { withAdapters?: boolean } = {}) {
        ...
        fs.writeFileSync(
            path.join(kitDir, MANIFEST_FILENAME),
            JSON.stringify(manifest, null, 2)
        );

        // Create pattern and checklist files
        fs.writeFileSync(
            path.join(patternsDir, 'api.error-model.md'),
            '# Error Model Pattern'
        );
        fs.writeFileSync(
            path.join(checklistsDir, 'checklist.api-review.md'),
            '# API Review Checklist'
        );

        // Create config files
        fs.writeFileSync(
            path.join(tempDir, 'bek.config.json'),
