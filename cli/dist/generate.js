...
function loadIndustryRules() {
    ...
    if (!rulesPath) {
        throw new CLIError('Industry rules not found', 'RULES_NOT_FOUND', 1, 'Run `bek sync` to update your kit or check your installation');
    }
...
export function detectIndustry(query) {
 ...
 */
export function detectIndustryFromProject(targetDir = process.cwd()) {
    const signals = [];
    let industryScores = {};
...
// =============================================================================
export function generateArchitecture(options) {
    ...
    if (!industry) {
        throw new CLIError(`Unknown industry: ${industryKey}`, 'UNKNOWN_INDUSTRY', 1, `Available: ${Array.from(rules.keys()).filter(k => !k.includes('-')).join(', ')}`);
    }
    return {
...
export function formatMarkdown(decision) {
...
// PERSIST TO FILE
// =============================================================================
export function persistArchitecture(decision, targetDir, page) {
