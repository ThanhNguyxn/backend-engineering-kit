...
function createDirectoryStructure(targetDir, template, dryRun) {
    const createdPaths = [];
    const dirs = [
...
function createSampleFiles(targetDir, template, dryRun) {
    const createdFiles = [];
    // Sample pattern
    const samplePattern = `---
id: sample-pattern
...
export async function initCommand(options = {}) {
    ...
    if (!template) {
        throw new CLIError(`Unknown template: ${templateKey}`, 'INVALID_TEMPLATE', 1, `Available: ${Object.keys(TEMPLATES).join(', ')}`);
    }
