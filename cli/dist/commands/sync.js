...
import { loadManifest, saveManifest, hasBackendKit, getCliVersion, BACKEND_KIT_DIR, } from '../lib/manifest.js';
...
export async function syncCommand(options = {}) {
    ...
    if (!hasBackendKit(target)) {
        throw new CLIError('No Backend Kit found in this directory', 'NO_BACKEND_KIT', 1, 'Run `bek init --preset <preset>` to initialize first');
    }
    // Load manifest
    const manifest = loadManifest(target);
    if (!manifest) {
        throw new CLIError('No manifest found. This project may have been initialized with an older version.', 'NO_MANIFEST', 1, 'Run `bek init --preset <preset> --force` to reinitialize with manifest support');
    }
    ...
    if (backup && !dryRun) {
        ...
        if (!json)
            logger.verbose(`  Backup: ${result.backupPath}`);
    }
    ...
    if (manifest.preset) {
        ...
        if (preset) {
            if (!json)
                logger.info(`Syncing preset: ${chalk.cyan(manifest.preset)}`);
